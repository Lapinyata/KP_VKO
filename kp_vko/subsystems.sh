	...
	then
		...
		do
			...
			then 
				...
				do
					if [[ "${TargetsId[0+8*$i]}" == "$IDTarget" ]] && ((${TargetsId[6+8*$i]} == 1)) # Если мы стреляли по цели и она вернулась
					then
						...
						do
							...
							then
								...
								then
									...
									then
										k=$(((${TargetsId[2+8*$cIdx]}-${YTarget})/(${TargetsId[1+8*$cIdx]}-${XTarget})))		# Формируем уравнение прямой
										b=$((${YTarget}-${k}*${XTarget}))
										...
										if (( $distace <= $r))					# Если расстояние от СПРО до прямой, по которой движеся цель, меньше радиуса зоны СПРО, то ...
										then
